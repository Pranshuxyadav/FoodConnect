...
<body>
    ...
    <section>
        ...
        <?php foreach ($rows as $row): ?>
            <li>
                <strong><?php echo $row['name']; ?></strong> - Status: 
                <span id="status-<?php echo $row['id']; ?>"><?php echo $row['status']; ?></span>
                
                <?php if (isset($_SESSION['role']) && $_SESSION['role'] == 'ngo' && isset($row['status']) && $row['status'] == 'pending'): ?>

                    <button onclick="acceptOrder(<?php echo $row['id']; ?>)">Accept Order</button>
                <?php endif; ?>
                
                <?php if (isset($_SESSION['role'], $row['status']) && $_SESSION['role'] == 'restaurant' && $row['status'] == 'waiting for approval'): ?>
